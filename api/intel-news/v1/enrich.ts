...
const SYSTEM_PROMPT = `You enrich a news article for a world events tracking app. Always return ONE JSON object with these fields:
  ...
  - country: the FULL ENGLISH NAME of the primary country in the story — e.g. "United States", "China", "Israel" (never a 2-letter code; codes get misassigned). ONLY include this field for stories about armed conflict, military operations, terrorist attacks, civil unrest, or any kinetic event with a clear geographic incident location. For other stories (business, sports, entertainment, etc.), OMIT this field entirely.
...
/* v9 (June 2026): country/countries output switched from ISO codes to full
 * English names — the LLM invented mnemonic codes (CH for China, IS for
 * Israel, NI for Nigeria) that are valid ISO for OTHER countries, silently
...
const LOCATION_ONLY_SYSTEM_PROMPT = `You classify a news article. Return ONE JSON object with these fields:
  ...
  - country: the FULL ENGLISH NAME of the primary country the story is about (its dateline / main location) — e.g. "United States", "China", "Israel", "Nigeria", "South Korea", "South Africa", "United Kingdom". NEVER use a 2-letter code: codes get misassigned (CH is Switzerland not China, IS is Iceland not Israel, NI is Nicaragua not Nigeria) and a wrong code silently routes the story to the wrong region. Include this for ANY story with a clear country focus — NOT just conflict (e.g. a US company → "United States", German politics → "Germany", a Tokyo event → "Japan"). OMIT only when the story is genuinely global or has no single country (e.g. an opinion piece on AI, or a multi-country summit with no clear host).

  - countries: an ARRAY of up to 3 FULL ENGLISH country names for the countries MATERIALLY involved — the primary country (same as "country") PLUS any others that are central actors or locations, primary first. Same rule: names, never codes. The point is to make a cross-border story reachable from EACH involved country's regional brief: "US arms shipment to Ukraine" → ["Ukraine","United States"]; "Iran–Israel strikes" → ["Iran","Israel"]; "US sanctions on Russia" → ["Russia","United States"]; "China–Taiwan tensions" → ["Taiwan","China"]. For a single-country story return just that one country. Return [] only for genuinely global stories with no country. ALWAYS fill this when "country" is set — at minimum it repeats the primary country.
